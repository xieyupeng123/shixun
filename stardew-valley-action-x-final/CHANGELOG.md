...
## V11 — 无敌模式重写（2026-06-28）

**代码量**：2876行 / 20个Python源文件（本次+48行）

### 🔄 无敌模式重写：自动过期 + 代价机制 + 冷却系统

#### 空格激活（仅开不可关）
- 按空格键**激活**无敌模式，无法手动关闭
- 激活时停止当前BGM，播放invincible.ogg循环
- 已无敌时再按空格无效

#### ⏱ 30秒倒计时显示
- 顶部居中显示 `INVINCIBLE 23.5s` 实时倒计时（1位小数）
- 金色文字+边框，每秒刷新
- 30秒后自动结束
...
- 无敌结束后进入**5分钟冷却期**
- 冷却期间按空格无效
- 顶部显示 `INVINCIBLE CD: 4:32` 灰色倒计时（分:秒格式）
- 冷却结束文字消失，可再次使用

#### 💀 结束代价
- 自动结束时扣除**当前血量50%**（至少保留1HP，不会致死）
- 自动结束时**清零全部能量值**
- 音乐自动恢复（根据区域判断Boss BGM还是普通BGM）

### 配置项（settings.py）
```
INVINCIBLE_DURATION = 30000    # 30秒持续时间
INVINCIBLE_COOLDOWN = 300000   # 5分钟冷却
INVINCIBLE_COST_HEALTH = 0.5   # 扣50%血量
INVINCIBLE_COST_ENERGY = 1.0   # 扣100%蓝量
```

---

## V10 Final（2026-06-26）
