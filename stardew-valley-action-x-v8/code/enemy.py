import pygame
from settings import monster_data, BOSS_PHASE_THRESHOLDS
...
class Enemy(Entity):
    def __init__(self, monster_name, pos, groups, obstacle_sprites,
        ...
        # graphics — use squid sprites, boss gets tinted
        self.import_graphics(monster_name)
        self.status = 'idle'
        self.image = self.animations[self.status][self.frame_index]
        self.rect = self.image.get_rect(topleft=pos)

        # boss has smaller hitbox relative to sprite
        if self.is_boss:
        ...
        self._phase_thresholds = BOSS_PHASE_THRESHOLDS
    ...
    def import_graphics(self, name):
        ...
        for animation in self.animations.keys():
            self.animations[animation] = import_folder(
                f'../graphics/monsters/{sprite_name}/' + animation)
    ...
    def actions(self, player):
        ...
        if self.status == 'attack':
            ...
            # boss does double damage in later phases
            ...
            if self.path:
                ...
                v = pygame.math.Vector2(np) - pygame.math.Vector2(self.rect.center)
                if v.length() < 4:
                    self.path.pop(0)
                    if self.path:
                        np = grid_to_pos(self.path[0], self.tile_size)
                        v = pygame.math.Vector2(np) - pygame.math.Vector2(self.rect.center)
                self.direction = v.normalize() if v.length() > 0 else pygame.math.Vector2()
            else:
    ...
    def _check_phase(self):
        ...
        for i, threshold in enumerate(self._phase_thresholds):
            if hp_ratio <= threshold and self._current_phase <= i:
                self._current_phase = i + 1
                # phase boost: speed increases
    ...
    def check_death(self):
        if self.health <= 0:
            ...
            if self.trigger_death_particles:
                name = 'squid' if self.monster_name == 'boss' else self.monster_name
                self.trigger_death_particles(self.rect.center, name)
    ...
    def animate(self, dt):
        ...
        # boss gets red tint in later phases
        if self.is_boss and self._current_phase >= 1:
            tint = pygame.Surface(self.image.get_size(), pygame.SRCALPHA)
            alpha = 60 * self._current_phase
            tint.fill((255, 0, 0, min(alpha, 180)))
