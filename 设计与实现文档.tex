...
package "控制层 (Presentation)" {
  [GameMain 游戏主类] as GM
  [MainMenu 主菜单] as MM
  [HUD 界面显示] as HUD
  [UpgradeMenu 升级菜单] as UM
  [DeathScreen 死亡界面] as DS
}

package "逻辑层 (Logic)" {
  [Level 关卡管理] as LV
  [MapLoader 地图加载] as ML
  [Camera 镜头管理] as CAM
  [CollisionSystem 碰撞系统] as CS
  [SpawnManager 生成管理] as SM
}

package "实体层 (Entity)" {
  [Player 玩家] as PL
  [Enemy 敌人] as EN
  [WeaponSprite 武器精灵] as WS
  [MagicSystem 魔法系统] as MS
  [ParticleManager 粒子管理] as PM
  [TileSprite 瓦片精灵] as TS
}

package "数据层 (Data)" {
  [Settings 配置数据] as SET
  [SaveManager 存档管理] as SV
  [CSVParser 地图解析] as CSV
  [AssetLoader 资源加载] as AL
}
...
abstract class "游戏精灵基类\n(Pygame Sprite)" as Sprite {
  +image: Surface
  +rect: Rect
  +pos: Vector2
  --
...
abstract class "实体基类\n(继承Sprite)" as Entity {
  ...
  #direction: Vector2
  #status: str
  #animation_index: float
  #frames: dict
  #hitbox: Rect
  --
  +{abstract} update(dt): void
  #move(dt): void
  #collision(walls): void
  #animate(dt): void
  #import_assets(): void
}
...
class "Player\n(继承Entity)" as Player {
  ...
  +update(dt): void
  +input(): void
  +attack(callback): void
  +magic(callback): void
  +get_damage(): int
  +receive_damage(amount): void
  +heal(amount): void
  +gain_exp(amount): void
  +upgrade(stat): void
}
...
class "Enemy\n(继承Entity)" as Enemy {
  ...
  -hurt_timer: float
  --
  +update(dt, player, walls): void
  -ai_state_machine(dt, player, walls): void
  -idle(): void
  -chase(player, walls): void
  -attack_target(player): void
  -find_path(target, walls): void
...
class "Level\n(关卡管理)" as Level {
  ...
  -map_data: dict
  --
  +__init__(map_id, player, on_transition): void
  +update(dt): void
  +render(): void
  -create_map(): void
  -create_enemies(): void
  -check_transitions(): void
  -handle_attack(direction): void
  -handle_magic(magic_type): void
}

class "MagicSystem" as Magic {
  -player: Player
  -particles: ParticleManager
  --
  +cast_heal(): void
  +cast_flame(direction, targets): void
}

class "ParticleManager" as Particles {
  -particles: list
  --
  +add(particle): void
  +update(dt): void
...
actor 玩家 as P
participant "GameMain" as G
participant "Level" as L
participant "Player" as PL
participant "WeaponSprite" as W
participant "Enemy" as E

P -> G : 按下空格键
G -> PL : input() 检测攻击输入
PL -> PL : 检查攻击冷却
PL -> L : on_attack(direction) 回调
L -> W : 创建武器精灵\n放在朝向方向
L -> L : 加入武器精灵组
...
loop 每帧更新
  ...
  L -> P : update(dt)
  L -> C : update(player)
  L -> L : check_transitions()
  alt 玩家到达传送点
    L -> G : on_transition(target_map) 回调
    G -> G : game_state = 'transitioning'
    loop 淡出动画
      G -> G : alpha 递增到255
      G -> G : 渲染黑色遮罩
    end
    G -> L : 销毁旧关卡
    G -> L : Level(target_map, player, on_transition)
    L -> L : create_map() 加载新地图
    L -> P : 放置到出生点
    loop 淡入动画
      G -> G : alpha 递减到0
      G -> G : 渲染黑色遮罩
...
loop 每帧更新
  ...
  alt 距离 > 警戒范围
    ...
    E -> E : 检查是否需要重新寻路
    alt 需要寻路
      E -> PA : find_path(player_pos, walls)
      PA -> PA : A*算法计算路径
      PA --> E : 返回路径列表
    end
    E -> E : 沿路径移动一步
...
skinparam state {
  ...
  BackgroundColor<<death>> Salmon
}

state "主菜单" as menu <<menu>>
state "游戏中" as game <<game>>
state "暂停/升级" as pause <<menu>>
state "死亡" as death <<death>>
state "场景切换中" as transition <<menu>>
state "存档对话框" as save <<menu>>
...
\begin{itemize}[leftmargin=2em]
    \item \textbf{地图渲染}：地面 → 草地 → 装饰 → 实体，四层叠加
    \item \textbf{视窗系统}：以玩家为中心截取地图局部，计算相机偏移量并限制在地图范围内
    \item \textbf{实体更新}：玩家 → 敌人 → 武器 → 魔法 → 粒子，按顺序更新
...
\begin{enumerate}[leftmargin=2em]
    ...
    \item \textbf{系统测试}：完整游戏流程测试（主菜单→游戏→死亡→重开）
