...
class CombatManager:
    """Handles weapon creation, magic, attack collision."""
    def __init__(self, level):
        self.level = level
        self.current_attack = None
        self.attack_sprites = pygame.sprite.Group()
        self.attackable_sprites = pygame.sprite.Group()
        self.destroyed_grass = []       # for save tracking
        self.killed_enemies = []        # for save tracking
        ...
        for atk in self.attack_sprites:
            ...
            for target in hits:
                if target.sprite_type == SPRITE_GRASS:
                    ...
                    dmg = self.level.player.attack_damage
    ...
    def damage_player(self, amount, attack_type='slash'):
        if self.level.player.vulnerable:
            self.level.player.take_damage(amount)
            self.level.animation_player.create_particles(
                attack_type, self.level.player.rect.center,
                [self.level.visible_sprites])
...
class BossManager:
    """Handles boss spawning, phases, kills, and victory trigger."""
    def __init__(self, level):
        self.level = level
        self.boss = None
...
class Level:
    def __init__(self, map_id='default', player=None, loaded_data=None):
        ...
        self.ui = UI()
        self.upgrade = None  # set after player creation
